...
pkgdatadir = $(datadir)/sber
pkgincludedir = $(includedir)/sber
pkglibdir = $(libdir)/sber
pkglibexecdir = $(libexecdir)/sber
...
CONFIG_HEADER = $(top_builddir)/src/config/sber-config.h
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	...
	$(top_srcdir)/src/config/sber-config.h.in \
...
ACLOCAL = ${SHELL} /home/cyperts/Downloads/SBERCOIN/build-aux/missing aclocal-1.15
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
ARFLAGS = cr
ARM_CRC_CXXFLAGS = 
AUTOCONF = ${SHELL} /home/cyperts/Downloads/SBERCOIN/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /home/cyperts/Downloads/SBERCOIN/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /home/cyperts/Downloads/SBERCOIN/build-aux/missing automake-1.15
AVX2_CXXFLAGS = -mavx -mavx2
AWK = mawk
BDB_CFLAGS = 
BDB_CPPFLAGS = 
BDB_LIBS = -ldb_cxx-4.8
BITCOIN_CLI_NAME = sber-cli
BITCOIN_DAEMON_NAME = sberd
BITCOIN_GUI_NAME = sber-qt
BITCOIN_TX_NAME = sber-tx
...
MAINT = 
MAKEINFO = ${SHELL} /home/cyperts/Downloads/SBERCOIN/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
...
OTOOL64 = 
PACKAGE = sber
PACKAGE_BUGREPORT = https://github.com/sber-project/sber/issues
PACKAGE_NAME = SBER Core
PACKAGE_STRING = SBER Core 4.3.99
PACKAGE_TARNAME = sber
PACKAGE_URL = https://sber.org/
PACKAGE_VERSION = 4.3.99
PATH_SEPARATOR = :
PIC_FLAGS = -fPIC
PIE_FLAGS = -fPIE
...
PKG_CONFIG_PATH = 
PORT = 
PROTOBUF_CFLAGS = -pthread -g -std=c++11 -DNDEBUG -I/usr/local/include
...
ZMQ_CFLAGS = 
ZMQ_LIBS = -lzmq
abs_builddir = /home/cyperts/Downloads/SBERCOIN
abs_srcdir = /home/cyperts/Downloads/SBERCOIN
abs_top_builddir = /home/cyperts/Downloads/SBERCOIN
abs_top_srcdir = /home/cyperts/Downloads/SBERCOIN
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /home/cyperts/Downloads/SBERCOIN/build-aux/install-sh
...
DIST_CONTRIB = $(top_srcdir)/contrib/sber-cli.bash-completion \
	       $(top_srcdir)/contrib/sber-tx.bash-completion \
	       $(top_srcdir)/contrib/sberd.bash-completion \
...
WINDOWS_PACKAGING = $(top_srcdir)/share/pixmaps/sber.ico \
  $(top_srcdir)/share/pixmaps/nsis-header.bmp \
  $(top_srcdir)/share/pixmaps/nsis-wizard.bmp \
  $(top_srcdir)/doc/README_windows.txt

LINUX_PACKAGING = $(top_srcdir)/share/pixmaps/sber16.xpm \
  $(top_srcdir)/share/pixmaps/sber32.xpm \
  $(top_srcdir)/share/pixmaps/sber128.png
...
COVERAGE_INFO = baseline.info \
  test_sber_filtered.info total_coverage.info \
  baseline_filtered.info functional_test.info functional_test_filtered.info \
  test_sber_coverage.info test_sber.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/SBER-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings \
...
src/config/sber-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/sber-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/sber-config.h
$(top_srcdir)/src/config/sber-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/sber-config.h src/config/stamp-h1
...
$(OSX_APP)/Contents/MacOS/SBER-Qt: all-recursive
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/SBER-Qt
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/SBER-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_sber.info: baseline_filtered.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c $(LCOV_OPTS) -d $(abs_builddir)/src -t test_sber -o $@
#	$(LCOV) -z $(LCOV_OPTS) -d $(abs_builddir)/src

#test_sber_filtered.info: test_sber.info
#	$(abs_builddir)/contrib/filter-lcov.py $(LCOV_FILTER_PATTERN) $< $@
#	$(LCOV) -a $@ $(LCOV_OPTS) -o $@

#functional_test.info: test_sber_filtered.info
...
#test_sber_coverage.info: baseline_filtered.info test_sber_filtered.info
#	$(LCOV) -a $(LCOV_OPTS) baseline_filtered.info -a test_sber_filtered.info -o $@

#total_coverage.info: test_sber_filtered.info functional_test_filtered.info
#	$(LCOV) -a $(LCOV_OPTS) baseline_filtered.info -a test_sber_filtered.info -a functional_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_sber.coverage/.dirstamp:  test_sber_coverage.info
...
#cov: test_sber.coverage/.dirstamp total.coverage/.dirstamp
...
clean-local: clean-docs
	rm -rf coverage_percent.txt test_sber.coverage/ total.coverage/ test/tmp/ cache/ $(OSX_APP)
