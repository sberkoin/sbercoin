...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/SBERParams"
    else
        PARAMS_DIR="$HOME/.sber-params"
    fi
fi
...
Failed to fetch the SBER zkSNARK parameters!
...
function main() {
    ...
    cat <<EOF
SBER - fetch-params.sh

This script will fetch the SBER zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common SBER zkSNARK parameters. Note that it is
