...
#if defined(NDEBUG)
#error "SBER cannot be compiled without assertions."
#endif
...
CAmount GetBlockValue(int nHeight)
{
    ...
    const int nLast = Params().GetConsensus().vUpgrades[Consensus::UPGRADE_ZC_V2].nActivationHeight; //Last PoW block
...
{
    util::ThreadRename("sber-scriptch");
...
bool CheckWork(const CBlock block, CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired) {
        // Sber Specific reference to the block with the wrong threshold was used.
        const Consensus::Params& consensus = Params().GetConsensus();
        if ((block.nTime == (uint32_t) consensus.nSberBadBlockTime) &&
                (block.nBits == (uint32_t) consensus.nSberBadBlockBits)) {
            // accept SBER block minted with incorrect proof of work threshold
            return true;
        }
