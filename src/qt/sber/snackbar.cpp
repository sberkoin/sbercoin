...
#include "qt/sber/forms/ui_snackbar.h"
#include "qt/sber/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(SBERGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &SBERGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
