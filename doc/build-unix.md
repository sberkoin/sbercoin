UNIX BUILD NOTES
====================
Some notes on how to build SBER Core in Unix.

Note
---------------------
Always use absolute paths to configure and compile SBER Core and the dependencies,
...
This will build sber-qt as well, if the dependencies are met.
...
memory available when compiling SBER Core. On systems with less, gcc can be
 ...
 **For Ubuntu only:** db4.8 packages are available [here](https://launchpad.net/~sber/+archive/sber).
 You can add the repository using the following command:

    sudo apt-get install software-properties-common
    sudo add-apt-repository ppa:sber/sber
    sudo apt-get update
...
To build SBER Core without wallet, see [*Disable-wallet mode*](/doc/build-unix.md#disable-wallet-mode)
...
If you want to build sber-qt, make sure that the required packages for Qt development
...
Once these are installed, they will be found by configure and a sber-qt executable will be
built by default.
...
Notes
-----
The release is built with GCC and then "strip sberd" to strip the debug
...
Security
--------
To help make your SBER Core installation more secure by making certain attacks impossible to
...
* _Non-executable Stack_: If the stack is executable then trivial stack-based buffer overflow exploits are possible if
    vulnerable buffers are found. By default, SBER Core should be built with a non-executable stack
...
When the intention is to run only a P2P node without a wallet, SBER Core may be compiled in
