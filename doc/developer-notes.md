...
- [Development guidelines](#development-guidelines)
    - [General SBER Core](#general-sber-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
SBER Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
SBER Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/sber-project/sber/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_sber
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_sber --log_level=test_suite
$ valgrind -v --leak-check=full src/sberd -printtoconsole
```
...
# A coverage report will now be accessible at `./test_sber.coverage/index.html`.
```
...
However, in open source software such as SBER Core, where everyone uses
...
pay attention to for reviewers of SBER Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: SBER Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against SBER Core so that they can be integrated
...
file descriptors it will cause problems with SBER's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:SBER-Project/SBER.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `sber-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `sber-cli` and the GUI debug console use this table to determine how to
