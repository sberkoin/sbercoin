...
SBER Core version *version* is now available from:  <https://github.com/sber-project/sber/releases>
...
SBER Core v5.0.0 is a mandatory update for all users. This release contains a protocol upgrade (to `70920`), new consensus rules and improvements that are not backwards compatible with older versions. Users will need to update their clients before enforcement of this update goes into effect.
...
#### Sapling Parameters
In order to run, SBER Core now requires two files, `sapling-output.params` and `sapling-spend.params` (with total size ~50 MB), to be saved in a specific location.
...
For the other packages, the user must save the param files in the proper location, before being able to run SBER v5.0.0:
- macOS/Linux `tar.gz` tarballs include a bash script (`install-params.sh`) to copy the parameters in the appropriate location.
- Windows `.zip` users need to manually copy the files from the `share/sber` folder to the `%APPDATA%\SBERParams` directory.
...
More visual information about the protocol can be found at https://sber.org .
...
See the release notes from [v4.2.0](https://github.com/SBER-Project/SBER/blob/master/doc/release-notes/release-notes-4.2.0.md#label-and-account-apis-for-wallet) for a full description of the changes from the 'account' API to the 'label' API.
...
| `getaccountaddress` | Returned the current SBER address associated with an internal account | Account system removal |
| `setaccount` | Associate a SBER address with an internal account name | Account system removal |
| `getaccount` | Returned the internal account associated with a given SBER address | Account system removal |
| `getaddressesbyaccount` | Returned a list of SBER addresses associated with an internal account | Account system removal |
...
| `sendfrom` | Send an amount from an internal account to a SBER address | Account system removal |
...
| `multisend` | Configure sending a portion of stake rewards to a SBER address | Pending a full rewrite |
...
### Build Systems
- #1856 `1bbb906059` [CMake] Add test_sber unit test target (Fuzzbawls)
...
- #2041 `a15dc3417d` [Build] CMake sberd and unit tests: fixing missing MacOS framework. (furszy)
