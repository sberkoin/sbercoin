...
*Setup instructions for a gitian build of SBER Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the SBER
...
to the SBER GitHub release page.
...
- [Building SBER Core](#building-sber-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/sber-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/sber-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/sber-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/sber-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/sber-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/sber-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/sber-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@sber.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/sber-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/sber-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/sber-osx-*-build.assert
```
